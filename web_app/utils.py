...
def predict_conversion(df:pd.DataFrame, numeric_columns, categorical_columns)->pd.DataFrame:
    ...
    # predict chonversion probabilities
    df['conversion_prob'] = model.predict_proba(df_hotenc)[:,1]

    # predict churn using churn threshold > 0.5
    churn_threshold = 0.3
    df['conversion'] = np.where(df['conversion_prob'].values > churn_threshold, 1, 0)
...
def pie_plot_cluster_distribution(df:pd.DataFrame):
    ...
    sizes = df['conversion'].value_counts().to_list()
